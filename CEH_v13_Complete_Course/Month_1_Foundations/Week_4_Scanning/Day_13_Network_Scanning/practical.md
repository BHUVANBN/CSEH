# Day 13 — Practical: Network Scanning Concepts
...
- **Tools:** Identified in topics: scanning methodology, TCP flags SYN ACK FIN RST PSH URG, firewall evasion fragmentation decoy idle, banner grabbing, masscan zmap comparison
...
```bash
mkdir -p ~/CEH_Labs/Week*/Day13
cd ~/CEH_Labs/Week*/Day13
```

### Phase 2: Core Practical Exercises

**3.** The main exercises for **Network Scanning Concepts** follow the methodology below.

Topics covered hands-on: **scanning methodology, TCP flags SYN ACK FIN RST PSH URG, firewall evasion fragmentation decoy idle, banner grabbing, masscan zmap comparison**
...
# Document all output in ~/CEH_Labs/Week*/Day13/output.txt
```

**5.** Document findings:
```bash
# Redirect important output to a file for your report
command_output 2>&1 | tee ~/CEH_Labs/Week*/Day13/findings.txt
```

### Phase 3: Tool-Specific Exercises

**6.** For each tool identified in the topics (scanning methodology, TCP flags SYN ACK FIN RST PSH URG, firewall evasion fragmentation decoy idle, banner grabbing, masscan zmap comparison):
...
```bash
sudo tcpdump -i eth1 -w day13_capture.pcap &
# Run your tools
kill %1
```
...
```bash
sudo wireshark day13_capture.pcap &
```
...
### Primary Tools for Network Scanning Concepts

```bash
# Tool syntax cheatsheet for: scanning methodology, TCP flags SYN ACK FIN RST PSH URG, firewall evasion fragmentation decoy idle, banner grabbing, masscan zmap comparison
