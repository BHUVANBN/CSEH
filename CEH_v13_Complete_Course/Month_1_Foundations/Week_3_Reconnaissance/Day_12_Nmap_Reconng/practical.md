# Day 12 — Practical: Nmap & Recon-ng
...
- **Tools:** Identified in topics: nmap scan types -sT -sS -sU -sN -sF -sX -sA -sI, OS detection -O, version -sV, NSE scripts, timing -T0-T5, output -oN -oX -oG, recon-ng workspaces modules
...
```bash
mkdir -p ~/CEH_Labs/Week*/Day12
cd ~/CEH_Labs/Week*/Day12
```

### Phase 2: Core Practical Exercises

**3.** The main exercises for **Nmap & Recon-ng** follow the methodology below.

Topics covered hands-on: **nmap scan types -sT -sS -sU -sN -sF -sX -sA -sI, OS detection -O, version -sV, NSE scripts, timing -T0-T5, output -oN -oX -oG, recon-ng workspaces modules**
...
# Document all output in ~/CEH_Labs/Week*/Day12/output.txt
```

**5.** Document findings:
```bash
# Redirect important output to a file for your report
command_output 2>&1 | tee ~/CEH_Labs/Week*/Day12/findings.txt
```

### Phase 3: Tool-Specific Exercises

**6.** For each tool identified in the topics (nmap scan types -sT -sS -sU -sN -sF -sX -sA -sI, OS detection -O, version -sV, NSE scripts, timing -T0-T5, output -oN -oX -oG, recon-ng workspaces modules):
...
```bash
sudo tcpdump -i eth1 -w day12_capture.pcap &
# Run your tools
kill %1
```
...
```bash
sudo wireshark day12_capture.pcap &
```
...
### Primary Tools for Nmap & Recon-ng

```bash
# Tool syntax cheatsheet for: nmap scan types -sT -sS -sU -sN -sF -sX -sA -sI, OS detection -O, version -sV, NSE scripts, timing -T0-T5, output -oN -oX -oG, recon-ng workspaces modules
