# Day 11 — Practical: DNS & WHOIS Enumeration
...
- **Tools:** Identified in topics: DNS record types A AAAA MX NS CNAME TXT SOA PTR, zone transfer AXFR, dnsrecon dnsenum fierce, WHOIS privacy, email header analysis SPF DKIM DMARC
...
```bash
mkdir -p ~/CEH_Labs/Week*/Day11
cd ~/CEH_Labs/Week*/Day11
```

### Phase 2: Core Practical Exercises

**3.** The main exercises for **DNS & WHOIS Enumeration** follow the methodology below.

Topics covered hands-on: **DNS record types A AAAA MX NS CNAME TXT SOA PTR, zone transfer AXFR, dnsrecon dnsenum fierce, WHOIS privacy, email header analysis SPF DKIM DMARC**
...
# Document all output in ~/CEH_Labs/Week*/Day11/output.txt
```

**5.** Document findings:
```bash
# Redirect important output to a file for your report
command_output 2>&1 | tee ~/CEH_Labs/Week*/Day11/findings.txt
```

### Phase 3: Tool-Specific Exercises

**6.** For each tool identified in the topics (DNS record types A AAAA MX NS CNAME TXT SOA PTR, zone transfer AXFR, dnsrecon dnsenum fierce, WHOIS privacy, email header analysis SPF DKIM DMARC):
...
```bash
sudo tcpdump -i eth1 -w day11_capture.pcap &
# Run your tools
kill %1
```
...
```bash
sudo wireshark day11_capture.pcap &
```
...
### Primary Tools for DNS & WHOIS Enumeration

```bash
# Tool syntax cheatsheet for: DNS record types A AAAA MX NS CNAME TXT SOA PTR, zone transfer AXFR, dnsrecon dnsenum fierce, WHOIS privacy, email header analysis SPF DKIM DMARC
